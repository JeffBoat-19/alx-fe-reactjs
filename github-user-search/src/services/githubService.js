...
try {
  ...
  if (raw) {
    if (/^https?:\/\//i.test(raw)) API_BASE = raw.replace(/\/$/, "");
      ...
      console.warn("[githubService] ignored non-absolute VITE_API_BASE*:", raw);
  }
} catch (e) {
...
async function fetchUserDetails(username) {
  ...
  try {
    ...
    console.warn(
      "[githubService] fetchUserDetails failed for",
      username,
...
async function searchGithubUsers({
  ...
  if (minRepos && Number(minRepos) > 0)
    qParts.push(`repos:>=${Number(minRepos)}`);

  if (qParts.length === 0) {
    throw new Error(
      "Empty search query — provide username, location, or minRepos."
    );
  }
  ...
  try {
    ...
    Object.entries(params).forEach(([k, v]) =>
      logUrl.searchParams.set(k, String(v))
    );
...
async function searchGithubRepos(query) {
  if (!query || !String(query).trim())
    throw new Error("Repository search requires a query.");
  try {
